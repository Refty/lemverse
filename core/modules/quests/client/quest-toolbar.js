...
const joinQuest = () => {
    ...
    Quests.update(questId, { $addToSet: { targets: userId } }, (error) => {
        if (error) {
            lp.notif.error(`Unable to join the task`)
            return
        }
...
const leaveQuest = () => {
    ...
    Quests.update(questId, { $pull: { targets: userId } }, (error) => {
        if (error) {
            lp.notif.error(`Unable to leave the task`)
            return
        }
...
const toggleQuestState = () => {
    ...
    Quests.update(quest._id, actions, (error) => {
        if (error) {
            lp.notif.error(`Unable to update the task`)
            return
        }
...
const createNewQuestFromTitle = (name) => {
    ...
    if (quest) {
        Quests.update(channel, { $set: { name } })
        return
    }
...
Template.questToolbar.events({
    'keypress input.js-quest-name': function (event) {
        if (event.which !== 13) return
        createNewQuestFromTitle(event.currentTarget.value)
    },
    'focus .js-quest-name': function (event) {
        event.preventDefault()
        event.stopPropagation()
        toggleUIInputs(true)
    },
    'blur .js-quest-name': function (event) {
        event.preventDefault()
        event.stopPropagation()
        toggleUIInputs(false)
        updateTitle(event.currentTarget.value)
    },
    'click .js-quest-join': function (event) {
        event.preventDefault()
        event.stopPropagation()
        joinQuest()
    },
    'click .js-quest-leave': function (event) {
        event.preventDefault()
        event.stopPropagation()
        leaveQuest()
    },
    'click .js-quest-invite': function (event, templateInstance) {
        event.preventDefault()
        event.stopPropagation()

        const quest = activeQuest()
        Session.set('modal', {
            template: 'userListSelection',
            selectedUsers: quest.targets,
            ignoredUsers: [quest.createdBy],
        })
    ...
    'click .js-toggle-state': function (event) {
        event.preventDefault()
        event.stopPropagation()
        toggleQuestState()
    },
...
Template.questToolbar.onCreated(function () {
    ...
    this.autorun(() => {
        ...
        Meteor.call('questUsers', quest._id, (error, users) => {
            if (error) {
                lp.notif.error(`An error occured while loading tasks`)
                return
            }
...
Template.questToolbar.helpers({
    show() {
        return Session.get('selectedQuestId')
    },
    title() {
    ...
    createdBy(user) {
        return activeQuest()?.createdBy === user._id
    },
    showJoinButton() {
        const quest = activeQuest()
        if (!quest) return false
        ...
        return (
            quest.origin.includes('ent_') &&
            !quest.targets.includes(Meteor.userId()) &&
            quest.createdBy !== Meteor.userId()
        )
    },
    showLeaveButton() {
        const quest = activeQuest()
    ...
    showInviteButton() {
        return showActionsButton() && !activeQuest()?.completed
    },
    showActionsButton() {
