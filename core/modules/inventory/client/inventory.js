...
Template.inventoryItemPanel.helpers({
    exist() {
        return Session.get('selectedInventoryItem')
    },
    name() {
        return Session.get('selectedInventoryItem')?.name
    },
    description() {
        return Session.get('selectedInventoryItem')?.description || '-'
    },
    amount() {
        return Meteor.user().inventory[Session.get('selectedInventoryItem')?._id] || 0
    },
    thumbnail() {
        return Session.get('selectedInventoryItem')?.thumbnail
    },
})

Template.inventoryItemPanel.events({
    'click .js-drop-item': function (event) {
        event.preventDefault()
        event.stopPropagation()

        const itemId = Session.get('selectedInventoryItem')._id
        const positionInFrontOfPlayer = userManager.getPositionInFrontOfCharacter(
            userManager.getControlledCharacter(),
            dropItemDistance
        )
        Meteor.call('dropInventoryItem', itemId, positionInFrontOfPlayer, (error, editedItems) => {
            if (error) {
                lp.notif.error('An error occured during the drop')
                return
            }
...
Template.inventoryItem.helpers({
    exist() {
        return Template.instance().item !== undefined && Meteor.user().inventory[this.item.itemId] > 0
    },
    amount() {
        return Meteor.user().inventory[this.item.itemId] || 0
    },
    thumbnail() {
        return Template.instance().item.thumbnail
    },
})

Template.inventoryItem.events({
    'click .js-inventory-item': function (event, templateInstance) {
        event.preventDefault()
        event.stopPropagation()
        Session.set('selectedInventoryItem', {
            ...templateInstance.item,
            amount: templateInstance.data.item.amount,
        })
    },
})
...
Template.inventory.helpers({
    hasItems() {
        return Template.instance().inventory.get().length
    },
    items() {
