...
Template.report.events({
    'change .js-report-reason-select': function (event) {
        Template.instance().reason.set(event.target.value)
    },
    'click .js-button-submit': function (event) {
        event.preventDefault()
        event.stopPropagation()
        const template = Template.instance()
        lp.notif.confirm(
            'Report user',
            `Do you really want to report this ${template.data.messageId ? 'message' : 'user'}?`,
            () => onSubmit(template.reason.get(), Meteor.users.findOne(template.data.userId), template.data.messageId)
        )
    },
})
