...
const formatedDuration = (value) => {
    ...
    const minutes = Math.floor((value % 3600) / 60)
        .toString()
        .padStart(2, '0')
    const seconds = Math.floor(Math.max(value % 60, 1))
        .toString()
        .padStart(2, '0')
...
Template.notificationsAudioPlayer.events({
    'click .js-play': function (event, templateInstance) {
        event.preventDefault()
        markNotificationAsRead(templateInstance.data._id)

        templateInstance._playing.set(true)
        if (templateInstance.audio.paused && templateInstance.audio.currentTime > 0 && !templateInstance.audio.ended)
            templateInstance.audio.play()
        else if (
            !templateInstance.audio.paused &&
            templateInstance.audio.currentTime > 0 &&
            !templateInstance.audio.ended
        ) {
...
Template.notificationButton.helpers({
    pendingNotificationsCount() {
        return Notifications.find({ read: { $exists: false } }).count()
    },
})
...
Template.notification.helpers({
    ...
    user() {
        return Meteor.users.findOne(Template.instance().data.createdBy)
    },
    type() {
        return getChannelType(this.channelId)
    },
    newQuest() {
        return this.type === 'quest-new'
    },
})

Template.notification.events({
    'click .js-notification-clickable': function (event) {
        event.preventDefault()
        markNotificationAsRead(this._id)
        window.dispatchEvent(
            new CustomEvent(eventTypes.onNotificationClicked, {
...
Template.notifications.events({
    'click .js-notification-mark-all-as-read': function (event) {
        event.preventDefault()
...
Template.notifications.helpers({
    notifications() {
        const notifsRead = Notifications.find(
            { read: true },
            { sort: { createdAt: -1 }, limit: notifsReadMaxDisplayed }
        ).fetch()
...
const userAvatar = async (user) => {
    ...
    const characterFileIds = Object.keys(charactersParts)
        .flatMap((part) => Characters.findOne(user.profile[part])?.fileId)
        .filter(Boolean)
    ...
    imageBitmaps.forEach((img) =>
        canvas.getContext('2d').drawImage(img, 48, 0, frameWidth, frameHeight, 16, 0, frameWidth, frameHeight)
    )
...
notify = async (userPoly, message) => {
    ...
    if (user) {
        try {
            ...
            log('failed to get user avatar', { err })
        }
