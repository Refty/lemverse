...
const allChannels = () => {
    ...
    const user = Meteor.user({
        fields: { 'profile.x': 1, 'profile.y': 1, 'profile.levelId': 1 },
    })
    if (!user) return []

    const sortedZones = zoneManager.findZonesForPosition({ x: user.profile.x, y: user.profile.y }).map((zone) => ({
        channel: zone._id,
        name: `📍 ${zone.name}`,
        priority: 1,
    }))
    ...
    if (nearUsersIds.length)
        nearUsersChannel = {
            channel: nearUsersIds,
            name: '👥 Near users',
            priority: 3,
        }
...
Template.messagesChannelSelector.events({
    'click .js-channel-selector': function (event) {
...
Template.messagesChannelSelector.helpers({
    active() {
        return Session.get('messagesChannel') === this.channel
    },
    channels() {
