...
const notifyQuestSubscribersAboutNewMessage = (questId, message) => {
    ...
    const usersToNotify = [...new Set([...subscribedUsers, ...targets])].filter(
        (userId) => userId !== message.createdBy
    )
    ...
    log('notifyQuestSubscribersAboutNewMessage: done', {
        amount: usersToNotify.length,
    })
}

const setCollectionLastMessageAtToNow = (collection, documentId) =>
    collection.update(documentId, { $set: { lastMessageAt: new Date() } })
...
const notifyUsers = (message) => {
    ...
    if (getChannelType(channel) === 'level') {
        userIds = Meteor.users
            .find(
                {
                    _id: { $ne: createdBy },
                    'status.online': true,
                    'profile.levelId': channel,
                },
                { fields: { 'profile._id': 1 } }
            )
            .fetch()
            .map((item) => item._id)
...
Meteor.methods({
    clearConferenceMessages(conferenceUUID) {
        ...
        log('clearConferenceMessages: start', {
            conferenceUUID,
            userId: this.userId,
        })
        ...
        log('clearConferenceMessages: done', {
            zoneId: zone._id,
            conferenceUUID,
            userId: this.userId,
        })
    ...
    sendMessage(channel, text, fileId) {
        ...
        analytics.track(this.userId, '✍️ Message Sent', {
            user_id: this.userId,
            context: getChannelType(channel),
        })
    ...
    toggleMessageReaction(messageId, reaction) {
        ...
        if (!message.reactions || !message.reactions[reaction]?.includes(this.userId)) {
            Messages.update(messageId, {
                $addToSet: { [`reactions.${reaction}`]: this.userId },
            })
        } else {
            Messages.update(messageId, {
                $pull: { [`reactions.${reaction}`]: this.userId },
            })
        }
