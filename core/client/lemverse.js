...
Template.lemverse.onCreated(function () {
            ...
            if (!this.handleObserveCharacters) {
                this.handleObserveCharacters = Characters.find().observe({
                    added(character) {
                        ...
                        bootScene.loadImagesAtRuntime([spritesheet], () =>
                            bootScene.loadCharacterAnimations([spritesheet])
                        )
                    },
        ...
        Tracker.nonreactive(() => {
            ...
            this.levelSubscribeHandler = this.subscribe('currentLevel', () => {
                ...
                window.dispatchEvent(
                    new CustomEvent(eventTypes.onLevelLoading, {
                        detail: { level },
                    })
                )
                ...
                this.handleTilesSubscribe = this.subscribe('tiles', levelId, () => {
                    this.handleObserveTiles = Tiles.find().observe({
                        added(tile) {
                            levelManager.onDocumentAdded(tile)
                        },
                        changed(newTile, oldTile) {
                            levelManager.onDocumentUpdated(newTile, oldTile)
                        },
                        removed(tile) {
                            levelManager.onDocumentRemoved(tile)
                        },
                    })
            ...
            const loadUsers = (onLoaded) => {
                Meteor.call('getUsers', Meteor.user()?.profile?.levelId, (err, data) => {
                    ...
                    if (err) {
                        error("Can't fetch users update")
                        return
                    }
                    if (!data) return
                    const newUsers = data.users.filter(
                        (user) => user.status?.online && user.profile.levelId === levelId
                    )
                    const oldUsers = Meteor.users
                        .find({
                            'status.online': true,
            ...
            loadUsers(() => {
                ...
                this.handleEntitiesSubscribe = this.subscribe('entities', levelId, () => {
                    this.handleObserveEntities = Entities.find({
                        levelId,
                        prefab: { $exists: false },
                    }).observe({
                        added(entity) {
                        ...
                        changed(newEntity, oldEntity) {
                            entityManager.onDocumentUpdated(newEntity, oldEntity)
                        },
                        removed(entity) {
                    ...
                    this.handleZonesSubscribe = this.subscribe('zones', levelId, () => {
                        this.handleObserveZones = Zones.find().observe({
                            added(zone) {
                                zoneManager.onDocumentAdded(zone)
                            },
                            changed(newZone, oldZone) {
                                zoneManager.onDocumentUpdated(newZone, oldZone)
                            },
                            removed(zone) {
                                zoneManager.onDocumentRemoved(zone)
                            },
                        })
    ...
    hotkeys('p', { keyup: true, scope: scopes.player }, (event) => {
        ...
        if (event.type === 'keydown' && !userProximitySensor.nearUsersCount()) {
            lp.notif.error(`You need someone near you to whisper`)
            return
        }
...
Template.lemverse.helpers({
    ...
    onboarding: () =>
        ...
        !Meteor.user({ fields: { username: 1 } })?.username,
