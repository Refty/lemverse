...
const onMenuOptionSelected = (e) => {
    ...
    else if (option.id === 'show-profile')
        Session.set('modal', {
            template: 'userProfile',
            userId: Session.get('menu')?.userId,
        })
    else if (option.id === 'go-back') buildMenuFromOptions([...mainMenuItems, ...additionalOptions('me')])
    else if (option.id === 'custom-reaction' && canUseLevelFeature(Meteor.user(), 'reactions', true))
        setReaction(Meteor.user().profile.defaultReaction || Meteor.settings.public.defaultReaction)
...
const computeMenuPosition = () => {
    ...
    return {
        x: (position?.x || 0) + menuOffset.x,
        y: (position.y || 0) + menuOffset.y,
    }
}
...
const onMouseMove = (event) => {
    ...
    const distance = Math.sqrt(
        (menuPosition.x - mousePosition.x) ** 2 + (menuPosition.y - radialMenuOffsetY - mousePosition.y) ** 2
    )
...
Template.radialMenuOption.helpers({
    isActive(value) {
        return Meteor.user()?.profile[value]
    },
    shortcutLabel() {
        return String.fromCharCode(this.shortcut)
    },
})
...
Template.radialMenu.onCreated(function () {
    ...
    hotkeys('1,2,3,4,5,6,7,8,9', { keyup: true, scope: scopes.player }, (e) => {
        ...
        if (e.type === 'keyup')
            window.dispatchEvent(
                new CustomEvent(eventTypes.onMenuOptionUnselected, {
                    detail: { option },
                })
            )
        else if (e.type === 'keydown')
            window.dispatchEvent(
                new CustomEvent(eventTypes.onMenuOptionSelected, {
                    detail: { option },
                })
            )
    })
    ...
    hotkeys('*', { keyup: true, scope: scopes.player }, (e) => {
        ...
        if (e.type === 'keyup')
            ...
            window.dispatchEvent(
                new CustomEvent(eventTypes.onMenuOptionSelected, {
                    detail: { option, user: menuCurrentUser() },
                })
            )
    })
...
Template.radialMenu.events({
    'mousedown .js-menu-item': function (event) {
        event.preventDefault()
        event.stopPropagation()
        window.dispatchEvent(
            new CustomEvent(eventTypes.onMenuOptionSelected, {
                detail: { option: this, user: menuCurrentUser() },
            })
        )
    },
    'mouseup .js-menu-item': function (event) {
        event.preventDefault()
        event.stopPropagation()
        window.dispatchEvent(
            new CustomEvent(eventTypes.onMenuOptionUnselected, {
    ...
    'mouseenter .js-menu-item': function (event, templateInstance) {
        templateInstance.label.set(this.label)
    },
    'mouseleave .js-menu-item': function (event, templateInstance) {
        templateInstance.label.set(undefined)
    },
})
...
Template.radialMenu.helpers({
    ...
    username() {
        return menuCurrentUser({ fields: { 'profile.name': 1 } })?.profile.name
    },
})
