...
const makeResizableDiv = (containerId) => {
    ...
    const resizeObserver = new ResizeObserver((entries) => {
        for (let idx = 0; idx < entries.length; ++idx) {
            const event = new CustomEvent(eventTypes.onElementResized, {
                detail: entries[idx],
            })
...
Template.resizable.events({
    'click .resizable #locked': function (event) {
    ...
    'click .resizable #unlocked': function (event) {
        event.preventDefault()
        event.stopPropagation()
    ...
    'click .resizable #switch': function (event) {
        event.preventDefault()
        event.stopPropagation()
