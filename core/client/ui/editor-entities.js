...
Template.editorEntity.helpers({
    ...
    thumbnail() {
        return generateEntityThumbnail(this, thumbnailMaxSize)
    },
})

Template.editorEntity.events({
    'click .validated': function (event, templateInstance) {
        const { checked } = event.currentTarget
        Entities.update(templateInstance.data._id, {
            [checked ? '$set' : '$unset']: { validated: true },
        })
    },
    'blur .entity-label': function (event, templateInstance) {
