...
Template.userListSelection.events({
    'click .js-selectable': function (event, templateInstance) {
    ...
    'click .js-submit': function (event, templateInstance) {
        const userList = Object.keys(templateInstance.selectedUsers.get())
        const msg = Meteor.settings.public.confirmMessage?.addUsers || `Are you sure to add these users to the team?`
...
Template.userListSelection.helpers({
    ...
    amountSelected() {
        return Object.keys(Template.instance().selectedUsers.get()).length
    },
})
