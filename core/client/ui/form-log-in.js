...
const onPasswordRecoverSubmit = (template) => {
    ...
    if (!email) {
        lp.notif.error('Who are you mister anonymous? 🤔')
        return
    }
...
const onSubmit = (template) => {
    ...
    if (!template.loginMode.get()) {
        onPasswordRecoverSubmit(template)
        return
    }
    if (Meteor.settings.public.passwordless) {
        passwordlessLogin(template)
        return
    }
    if (!password) {
        lp.notif.error(`Hey! I need your password... Promise, I won't share it with anyone 🤐`)
        return
    }

    const { x: ghostX, y: ghostY, direction: ghostDirection, levelId: ghostLevelId } = Meteor.user().profile

    Meteor.loginWithPassword(email, password, (err) => {
        if (err) {
            lp.notif.error('Incorrect login or password')
            return
        }
...
Template.formLogIn.events({
    ...
    'keyup .js-email': function (event, templateInstance) {
        templateInstance.email = event.target.value
    },
    'keyup .js-password': function (event, templateInstance) {
        templateInstance.password = event.target.value
    },
    'click .js-cancel-login-mode': function () {
        Template.instance().loginMode.set(true)
    },
    'click .js-password-lost': function () {
        Template.instance().loginMode.set(false)
    },
...
Template.formLogIn.helpers({
    ...
    contactURL() {
        return Meteor.settings.public.permissions?.contactURL
    },
    emailSent() {
