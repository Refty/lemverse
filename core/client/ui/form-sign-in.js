...
const checkPassword = (value) => {
    ...
    if (value.length < 10 && value.replace(/[a-z]/gm, '').length < 3)
        return 'Your password should be more than 9 characters or contains at least 3 special characters (numbers, upper case characters or symbols)'
...
const onSubmit = (template) => {
    ...
    if (step < 4) {
        ...
        Meteor.call(
            'convertGuestAccountToRealAccount',
            template.email,
            template.nickname,
            template.password,
            source,
...
Template.formSignIn.events({
    ...
    'click .js-previous-step': function () {
        previousStep(Template.instance())
    },
    ...
    'keyup .js-nickname': function (event, templateInstance) {
        templateInstance.nickname = event.target.value
    },
...
Template.formSignIn.helpers({
    ...
    nickname() {
        return Template.instance().nickname
    },
    password() {
