...
const onDragEnd = function () {
    Entities.update(this.name, { $set: { x: this.x, y: this.y } })
}
const onPointerDown = function () {
    Session.set('selectedEntityId', this.name)
}

const addEntityAnchor = (scene, container, entity) => {
    const anchor = scene.add
        .sprite(entity.x, entity.y, 'circle')
        .setName(entity._id)
...
Template.entityToolbox.helpers({
    showEntityList() {
        return !Session.get('selectedEntityId')
    },
    customEntityUploadAllowed() {
        return customEntityUploadAllowed()
    },
    entities() {
...
Template.entityToolbox.events({
    'change .js-entity-sprite-upload': function (event) {
        ...
        const uploadedFile = Files.insert(
            {
                file,
                meta: { source: 'toolbox-entity', userId: Meteor.userId() },
            },
            false
        )
        uploadedFile.on('end', (error, fileDocument) => {
            if (error) {
                lp.notif.error(`Error during file upload: ${error.reason}`)
                return
            }
    ...
    'click .js-open-entity-template-list': function () {
        Session.set('modal', { template: 'entityTemplateList' })
    },
    'click .js-open-entity-editor': function () {
        Session.set('selectedEntityId', this._id)
    },
    'click .js-create-new-prefab': function () {
        createNewPrefab()
