...
const prefabHideProperties = ['_id', 'levelId', 'createdAt', 'createdBy']
...
Template.entityTemplateEntry.helpers({
    ...
    thumbnail() {
        return generateEntityThumbnail(this)
    },
})
...
Template.entityEditPrefab.helpers({
    properties() {
        ...
        prefabHideProperties.forEach((property) => {
    ...
    name() {
        return this.entity.name || this.entity._id
    },
})

Template.entityTemplateList.events({
    'click .js-entity-entry': function () {
    ...
    'click .js-entity-edit': function () {
        Session.set('modal', { template: 'entityEditPrefab', entity: this })
    },
})

Template.entityEditPrefab.events({
    'click .js-prefab-delete': function () {
        lp.notif.confirm(
            'Entity prefab deletion',
            `Are you sure to delete the prefab "<b>${this.entity.name || this.entity._id}</b>"?`,
            () => {
    ...
    'click .js-prefab-delete-all': function () {
        ...
        lp.notif.confirm(
            ...
            `Are you sure to delete ${instances.length} instance(s) of "<b>${
                this.entity.name || this.entity._id
            }</b>"?`,
            () => {
    ...
    'click .js-prefab-cancel': function () {
        closeModal()
    },
    'click .js-prefab-version': function () {
        let values
        try {
            values = JSON.parse($('.entity-toolbox-properties textarea').val())
        } catch (err) {
            lp.notif.error(`invalid JSON format`, err)
        }
