...
Template.zonesToolboxProperties.helpers({
    properties() {
        ...
        zoneHideProperties.forEach((property) => {
    ...
    name() {
        return this.zone.name || this.zone._id
    },
})

Template.zonesToolboxProperties.events({
    'click .js-zone-cancel': function () {
        Session.set('displayZoneId', undefined)
    },
    'click .js-zone-save': function () {
    ...
    'click .js-zone-delete': function () {
        lp.notif.confirm(
            'Zone deletion',
            `Are you sure to delete the zone "<b>${this.zone.name || this.zone._id}</b>"?`,
            () => {
...
Template.zonesToolbox.onCreated(function () {
    this.autorun(() => {
        ...
        if (zoneId)
            Session.set('modal', {
                template: 'zonesToolboxProperties',
...
Template.zonesToolbox.onRendered(function () {
    this.autorun(() => {
        ...
        Zones.find().forEach((zone) => {
            ...
            const r = game.scene.keys.WorldScene.add.rectangle(
                zone.x1,
                zone.y1,
                zone.x2 - zone.x1,
                zone.y2 - zone.y1,
...
Template.zonesToolbox.helpers({
    ...
    displayZone() {
        return Zones.findOne(Session.get('displayZoneId'))
    },
})

Template.zonesToolbox.events({
    'click .js-zone-add': function () {
        const { levelId } = Meteor.user().profile
        const zoneId = Zones.insert({
            _id: Zones.id(),
            createdAt: new Date(),
