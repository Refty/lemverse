...
const targets = () => {
    ...
    return Entities.find({
        _id: { $ne: activeEntityId },
        actionType: { $ne: entityActionType.pickable },
    }).fetch()
}
...
Template.entityEditor.events({
    ...
    'input #entity-depth': function (event) {
        const entity = selectedEntity()
        if (!entity) return

        let { valueAsNumber: value } = event.target
        if (value >= 6) value += layerOffset

        Entities.update(entity._id, {
            $set: {
                'gameObject.depth': clamp(value, entityDepthRange.min, entityDepthRange.max + layerOffset),
            },
        })
    },
    'input #entity-scale': function (event) {
        const entity = selectedEntity()
        if (!entity) return
        ...
        if (value !== 0)
            Entities.update(entity._id, {
                $set: {
                    'gameObject.scale': clamp(value, -entityMaxScale, entityMaxScale),
                },
            })
    },
    'click .js-reset-attribute': function (event) {
        const entity = selectedEntity()
        if (!entity) return
    ...
    'change #js-entity-target': function (event) {
        Meteor.call('updateEntityTarget', Session.get('selectedEntityId'), event.target.value)
    },
    'blur .js-edit-entity-name': function (event) {
        event.preventDefault()
        event.stopPropagation()
        toggleUIInputs(false)

        Entities.update(Session.get('selectedEntityId'), {
            $set: { name: event.currentTarget.value },
        })
    },
    'focus .js-edit-entity-name': function (event) {
        event.preventDefault()
        event.stopPropagation()
...
Template.entityEditor.helpers({
    ...
    flipped() {
        return selectedEntity()?.gameObject.scale < 0
    },
    state() {
        return linkedEntity()?.state || selectedEntity()?.state
    },
    targets() {
        return targets()
    },
    hasSprite() {
        return Boolean(selectedEntity()?.gameObject?.sprite)
    },
    isActionable() {
        return Boolean(selectedEntity()?.actionType === entityActionType.actionable)
    },
    hasCustomAction() {
        return Boolean(selectedEntity()?.action)
    },
})
