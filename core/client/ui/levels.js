...
const levelQueryFilters = (ignoredLevelId) => ({
    ...
    $or: [{ $or: [{ hide: false }, { hide: { $exists: false } }] }, { createdBy: Meteor.userId() }],
})
...
Template.levels.onCreated(function () {
    ...
    this.autorun(() => {
        const levels = Levels.find(levelQueryFilters(), {
            fields: { createdBy: 1 },
        }).fetch()
...
Template.levels.events({
    'click .js-create-level': function (event, templateInstance) {
    ...
    'click .js-tab-switcher': function (event, templateInstance) {
        const { mode } = event.target.dataset
        templateInstance.tab.set(mode)
    },
    'click .js-level-select': function (event, templateInstance) {
...
Template.levels.helpers({
    isLevelOwner(level) {
        return isLevelOwner({ _id: Meteor.userId() }, level)
    },
    levels() {
        const currentLevelId = Meteor.user({ fields: { 'profile.levelId': 1 } })?.profile.levelId
        const levels = Levels.find(levelQueryFilters(currentLevelId), {
            sort: { visit: -1 },
        }).fetch()
    ...
    levelName(level) {
        ...
        if (!user) return `Guest's world`

        return `${user.profile.name}'s world`
    },
    levelVisitCount(level) {
        return level.visit || 0
    },
    loading() {
        return Template.instance().loading.get()
    },
    showList() {
        return Template.instance().tab.get() === listMode
    },
    showCreate() {
        return Template.instance().tab.get() !== listMode
    },
