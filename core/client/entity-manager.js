...
entityManager = {
    ...
    onDocumentUpdated(newEntity, oldEntity) {
        ...
        window.dispatchEvent(
            new CustomEvent(eventTypes.onEntityUpdated, {
    ...
    interactWithNearestEntity() {
        ...
        if (this.previousNearestEntity.action) {
            ...
            window.dispatchEvent(
                new CustomEvent(eventTypes.onEntityInteractionStarted, {
                    detail: { entity: this.entityInUse, action, value },
                })
            )
            ...
            if (action === 'modal')
                Session.set('modal', {
                    template: value,
                    entity: this.previousNearestEntity,
                })
        ...
        this.scene.tweens.add({
            ...
            onComplete: () => {
                Meteor.call('useEntity', _id, (error) => {
                    if (error) {
                        lp.notif.error(itemAlreadyPickedText)
                        return
                    }
    ...
    handleNearestEntityTooltip(position) {
        ...
        if (nearestEntity && this.entityDistanceTo(nearestEntity, position) >= entityTooltipConfig.proximityRequired)
    ...
    stopInteracting() {
        ...
        URLOpener.close()
        window.dispatchEvent(
            new CustomEvent(eventTypes.onEntityInteractionStopped, {
                detail: { entity: this.entityInUse },
            })
        )
    ...
    spawnEntities(entities, callback) {
        ...
        bootScene.loadImagesAtRuntime(sprites, () => {
            entities.forEach((entity) => {
                ...
                window.dispatchEvent(
                    new CustomEvent(eventTypes.onEntityAdded, {
    ...
    createAnimationsFromConfig(sprite, config) {
        Object.entries(config).forEach(([key, animConfig]) => {
            this.scene.anims.create({
                key: `${sprite.key}-${key}`,
                frames: this.scene.anims.generateFrameNames(sprite.key, {
                    ...animConfig,
                }),
    ...
    spawnColliderFromConfig(gameObject, config) {
        ...
        const { radius, width, height, x, y, immovable, bounce, dragX, dragY, damping, worldBounds, collideTilemap } =
            config
    ...
    computeTooltipPosition(entity) {
        ...
        return {
            x: 0,
            y: -(mainSprite.displayHeight + (pickable ? floatingDistance : 0)),
        }
    },
