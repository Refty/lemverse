...
EditorScene = new Phaser.Class({
        ...
        if (this.mode === editorModes.zones) {
            ...
            if (this.isMouseDown && !this.input.manager.activePointer.isDown) {
                ...
                if (zoneId) {
                    ...
                    if (!zone) return
                    const { startPosition, endPosition } = this.computePositions(
                        zone,
                        worldPoint,
                        Session.get('selectedZonePoint'),
                        altIsDown
                    )
                    ...
                    if (mapSelectedTileset) {
                        ...
                        for (let x = 0; x < selectedTiles.w; x++) {
                            for (let y = 0; y < selectedTiles.h; y++) {
                                ...
                                const globalSelectedTileIndex = levelManager.tileGlobalIndex(
                                    mapSelectedTileset,
                                    selectedTileIndex
                                )
                        ...
                        for (let y = 0; y < selection.h; y++) {
                            ...
                            for (let layer = 0; layer < layers; layer++) {
                                ...
                                if ((previousTile && previousTile.index !== tileToCopy.index) || !previousTile) {
                                    map.putTileAt(tileToCopy.index, currentTileX, currentTileY, false, layer)?.setAlpha(
                                        0.65
                                    )
                                }
                            }
                ...
                for (let x = 0; x < selection.w; x++) {
                    for (let y = 0; y < selection.h; y++) {
                        ...
                        for (let layer = 0; layer < map.layers.length; layer++) {
                            ...
                            const databaseTilesetToCopy = Tilesets.findOne(
                                { gid: { $lte: tileToCopy.index } },
                                { sort: { gid: -1 } }
                            )
                            ...
                            if (tileToUpdate && tileToCopy.index !== tileToUpdate.index) {
                                const databaseTileToUpdate = getDatabaseTile(
                                    pointerOffsetX,
                                    pointerOffsetY,
                                    tileToUpdate
                                )
                                ...
                                Tiles.update(databaseTileToUpdate._id, {
                                    $set: {
                                        createdAt: new Date(),
            ...
            entities.forEach((entity) => {
                if (entity.gameObject?.collider) {
                    ...
                    if (entity.gameObject.collider.radius) {
                        ...
                        collision = {
                            x: entity.gameObject.collider.x,
                            y: entity.gameObject.collider.y,
                            w: entity.gameObject.collider.width,
                            h: entity.gameObject.collider.height,
                        }
                    }
    ...
    computePositions(zone, mousePosition, editedPoint, snapPositions = false) {
        ...
        let startPosition = {
            x: zone.x1 || mousePosition.x,
            y: zone.y1 || mousePosition.y,
        }
        let endPosition = {
            x: zone.x2 || mousePosition.x,
            y: zone.y2 || mousePosition.y,
        }
