...
class Character extends Phaser.GameObjects.Container {
    ...
    enablePhysics(value = true) {
        if (value) {
            ...
            this.body.setOffset(
                configuration.colliderConfiguration.offset.x,
                configuration.colliderConfiguration.offset.y
            )
    ...
    setTintFromState() {
        const currentZone = zoneManager.currentZone({
            profile: { x: this.x, y: this.y },
        })
        const color = currentZone?.disableCommunications
            ? configuration.colorStates.unavailable
            : configuration.colorStates.default
        this.setTint(color)
    }
    ...
    physicsStep() {
        ...
        if (this.followedGameObject) {
            const diff = {
                x: this.followedGameObject.x - this.x,
                y: this.followedGameObject.y - this.y,
            }
    ...
    _createStateIndicator() {
        const muteIndicatorMic = this.scene.add
            .text(0, -40, '🎤', { font: '23px Sans Open' })
            .setDepth(99996)
            .setOrigin(0.5, 1)
        const muteIndicatorCross = this.scene.add
            .text(0, -40, '🚫', { font: '23px Sans Open' })
            .setDepth(99995)
            .setOrigin(0.5, 1)
            .setScale(0.8)
