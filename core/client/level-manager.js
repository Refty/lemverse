...
levelManager = {
    ...
    onDocumentUpdated(newTile) {
        ...
        this.map
            .putTileAt(this.tileGlobalIndex(tileset, newTile.index), newTile.x, newTile.y, false, layer)
            ?.setAlpha(1)
        window.dispatchEvent(
            new CustomEvent(eventTypes.onTileChanged, {
                detail: { tile: newTile, layer },
            })
        )
    },
    ...
    tileRefresh(x, y) {
        ...
        Tiles.find({ x, y }).forEach((tile) => {
            const tileset = this.map.getTileset(tile.tilesetId) || defaultTileset
            this.map.putTileAt(
                this.tileGlobalIndex(tileset, tile.index),
                tile.x,
                tile.y,
                false,
                this.tileLayer(tileset, tile.index)
            )
        })
    },
    ...
    tileGlobalIndex(mapTileset, tileIndex) {
        return (mapTileset.firstgid || 0) + tileIndex
    },

    tileLayer(mapTileset, tileIndex) {
        return mapTileset.tileProperties[tileIndex]?.layer ?? defaultTileset.layer
    },
    ...
    onLevelLoaded(levelDocument) {
        ...
        setTimeout(
            () =>
                game.scene.keys.LoadingScene.hide(() => {
                    this.scene.enableKeyboard(true)
                    window.dispatchEvent(
                        new CustomEvent(eventTypes.onLevelLoaded, {
                            detail: { level: levelDocument },
                        })
                    )
                }),
            0
    ...
    onLevelUnloaded(levelDocument) {
        window.dispatchEvent(
            new CustomEvent(eventTypes.onLevelUnloaded, {
                detail: { level: levelDocument },
            })
        )
    },
    ...
    onTilesetUpdated(newTileset, oldTileset) {
        ...
        const xys = _.map(
            Tiles.find({
                tilesetId: newTileset._id,
                index: { $in: changedTileIndexes },
            }).fetch(),
            (t) => ({ x: t.x, y: t.y })
        )
        ...
        const disabledCollisionGlobalIndexes = _.map(disabledCollisionIndexes, (i) =>
            this.tileGlobalIndex(newTileset, i)
        )
    ...
    drawTriggers(state) {
        ...
        const zones = Zones.find({
            targetedLevelId: { $exists: true, $ne: '' },
        }).fetch()
        zones.forEach((zone) => {
            const graphic = this.scene.add.rectangle(
                zone.x1,
                zone.y1,
                zone.x2 - zone.x1,
                zone.y2 - zone.y1,
