...
userStreams = {
    ...
    audio(enabled) {
        this.streams.main.instance?.getAudioTracks().forEach((track) => {
            track.enabled = enabled
        })
    },

    video(enabled) {
        this.streams.main.instance?.getVideoTracks().forEach((track) => {
            track.enabled = enabled
        })
    },
    ...
    async requestUserMedia(constraints = {}) {
        ...
        try {
            ...
            Meteor.users.update(Meteor.userId(), {
                $set: { 'profile.userMediaError': err.message },
            })
            ...
            this.streams.main.loading = false
        }

        debug('requestUserMedia: stream created', {
            streamId: stream.id,
            constraints,
        })
        this.streams.main.instance = stream
        window.dispatchEvent(
            new CustomEvent(eventTypes.onMediaStreamStateChanged, {
                detail: { type: streamTypes.main, state: 'ready', stream },
            })
        )
        Meteor.users.update(Meteor.userId(), {
            $unset: { 'profile.userMediaError': 1 },
        })
    ...
    async requestDisplayMedia() {
        ...
        try {
            ...
            this.streams.screen.loading = false
        }
        ...
        window.dispatchEvent(
            new CustomEvent(eventTypes.onMediaStreamStateChanged, {
                detail: { type: streamTypes.screen, state: 'ready', stream },
            })
        )
    ...
    async createStream(forceNew = false) {
        ...
        const { shareVideo, shareAudio } = Meteor.user({
...
export { screenShareDefaultConfig, videoDefaultConfig }
