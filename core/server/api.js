const getLevel = (params, req, res) => {
    ...
    if (params?.query?.access_token) apiKey = params.query.access_token
        ...
        ;({ 1: apiKey } = Buffer.from(auth || '', 'base64')
            .toString('utf8')
            .split(':'))
    }
...
lp.route('/api/entities/:entityId', 'application/json', 'api', (params, req, res) => {
    try {
        check(params.entityId, Match.SafeString)
    } catch (err) {
...
lp.route('/api/zones/:zoneId/messages', 'application/json', 'api', (params, req, res) => {
    try {
        check(params.zoneId, Match.SafeString)
    } catch (err) {
...
lp.route('/api/hooks', 'application/json', 'api', (params, req, res) => {
    ...
    if (level.hooks && level.hooks.find((h) => h.targetUrl === targetUrl)) {
        res.writeHead(409)
        return 'duplicate hooks'
    }
    ...
    log('/api/hooks: new hooks registered', {
        levelId,
        reqBody: req.body,
        hook,
    })
