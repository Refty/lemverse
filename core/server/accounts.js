...
Accounts.onLogin((param) => {
    ...
    log('onLogin: start', {
        ...
        userAgent: param.connection?.httpHeaders?.['user-agent'],
        languages: param.connection?.httpHeaders?.['accept-language'],
    })
    ...
    if (!isBodyValid) {
        log('onLogin: setting default skin', {
            userId: user._id,
            ip: param.connection?.httpHeaders?.['x-forwarded-for'],
            userAgent: param.connection?.httpHeaders?.['user-agent'],
            languages: param.connection?.httpHeaders?.['accept-language'],
...
Accounts.validateLoginAttempt((param) => {
    ...
    log('validateLoginAttempt: start', {
        type: param.type,
        allowed: param.allowed,
        methodName,
        username: param.methodArguments?.[0].user?.username,
        error: param.error,
    ...
    if (Meteor.settings.forbiddenIPs?.includes(lp.ip(param).ip)) {
        error('validateLoginAttempt: watched ip detected!', {
            ip: lp.ip(param).ip,
            userId: user?._id,
        })
        return false
    ...
    if (user?.disabled) {
        log('validateLoginAttempt: user account is disabled', {
            userId: user._id,
        })
        return false
    }
...
Meteor.server.method_handlers.requestLoginTokenForUser = (options) => {
    ...
    if (user?.disabled) {
        log('RequestLoginTokenForUser: account is disabled', {
            userId: user._id,
        })
