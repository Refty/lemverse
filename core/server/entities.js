...
const applyEntityState = (entity, stateActions) => {
    ...
    stateActions.replace?.forEach((t) =>
...
const pickEntityInventory = (entity) => {
    ...
    const itemsToAdd = inventoryItemKeys.map((key) => ({
        itemId: key,
        amount: entity.inventory[key],
    }))
...
Meteor.methods({
    ...
    spawnEntityFromFile(fileId, options = {}) {
        ...
        if (!lp.isLemverseBeta('custom-sprite'))
            throw new Meteor.Error('invalid-user', 'available for admin only for now')
