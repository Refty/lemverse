...
createLevel = (options) => {
    ...
    analytics.track(user._id, '🐣 Level Created', {
        level_id: newLevelId,
...
deleteLevel = (levelId) => {
    ...
    Meteor.users.update(
        ...
        { $set: { 'profile.levelId': Meteor.settings.defaultLevelId } },
        { multi: true }
    )
}
...
Meteor.publish('levelTemplates', function () {
    ...
    return Levels.find(
        { template: true, hide: { $exists: false } },
        { fields: { name: 1, hide: 1, visit: 1, createdBy: 1, template: 1 } }
    )
})
