...
spritesheetValid = (fileRef) => {
    ...
    const linkedImageAssets = Files.find({
        name: { $in: linkedImageNames },
    }).fetch()
...
importSpritesheetFramesAsEntities = (fileRef) => {
    ...
    data.textures.forEach((texture) => {
        texture.frames.forEach((frame) => {
            ...
            const existingEntity = Entities.findOne({
                name,
                'gameObject.sprite.assetId': asset._id,
                prefab: true,
            })
            if (existingEntity) {
                log('importSpritesheetFramesAsEntities: updating entity', {
                    name,
                    assetId: asset._id,
                })
